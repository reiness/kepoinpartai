                ...
                <tbody>
                    @foreach ($partaiData as $index => $partai)
                        <tr>
                            ...
                            <td class="editable" data-field="jumlah_kasus_suap_gratifikasi">
                                {{ $partai->jumlah_kasus_suap_gratifikasi }}</td>
                            <td class="editable" data-field="nominal_suap_gratifikasi">
                                {{ $partai->nominal_suap_gratifikasi }}</td>
                            <td class="editable" data-field="nominal_kasus_korupsi">{{ $partai->nominal_kasus_korupsi }}
                            </td>
                            ...
                            <td>
                                <button class="btn btn-primary edit-button" data-index="{{ $index }}"
                                    data-id="{{ $partai->id_partai }}">Edit</button>
    ...
    <script>
        ...
        function saveChanges(index) {
            ...
            const partai = {
                ...
                jumlah_kasus_suap_gratifikasi: rows[index].querySelector('[data-field="jumlah_kasus_suap_gratifikasi"]')
                    .textContent,
                nominal_suap_gratifikasi: rows[index].querySelector('[data-field="nominal_suap_gratifikasi"]')
                    .textContent,
                nominal_kasus_korupsi: rows[index].querySelector('[data-field="nominal_kasus_korupsi"]').textContent,
            ...
            fetch(`{{ url('admin/updatePartai') }}/${partai.id_partai}`, {
                ...
                .then((data) => {
                    if (data.success) {
                        ...
                        // Re-enable editing by removing the contentEditable and border
                        const row = rows[index].querySelectorAll('.editable');
                        row.forEach(cell => {
                            cell.contentEditable = false;
                            cell.style.border = 'none';
                        });
